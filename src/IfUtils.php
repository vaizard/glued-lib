...
class IfUtils
{
    ...
    public function getDeploymentsJson($service = false): string {
        ...
        $q = "SELECT json_agg(json_build_object('uuid', uuid, 'conn', conn, 'meta', meta, 'uri', concat('{$base}',service,'/latest/',uuid))) AS deployments FROM if__deployments";
    ...
    public function getDeploymentsArr($service = false): array {
        return json_decode($this->getDeploymentsJson($service));
    }
